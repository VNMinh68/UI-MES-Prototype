/* Bo tach build/legacy/script.js (ban 1 file cu) thanh app/shared/core.js +
 * app/sewing/script.js + app/finishing/script.js.
 *
 * Chay:  node build/emit.js
 *
 * Cach lam: KHONG go lai code. Doc build/members.json (ban do tung thanh vien cua
 * class Component, sinh boi build/members.js) roi CAT nguyen van tung khoi sang file dich
 * theo bang phan nhom ben duoi. Nhung thanh vien phai viet lai (constructor, restore,
 * cac ham doc du lieu tu PSCHED/KHC...) nam trong build/parts/*.js va duoc chen vao.
 */
'use strict';
const fs = require('fs');
...
/* Khoa moi sinh ra khi tach module (ten module + duong ve trang chon module). */
const EXTRA = {
  core: { vi: { modBack: "'Chọn module'", modSew: "'May'", modFin: "'Hoàn thiện'" },
          en: { modBack: "'Switch module'", modSew: "'Sewing'", modFin: "'Finishing'" } },
...
function out(rel, text) {
  ...
  console.log(rel.padEnd(24), text.split('\n').length + ' dong');
}
...
out('sewing/script.js', [
  part('sew-head.js'),
  LFor('sew', 'LMOD'),
  navviFor(['SEWING', 'Sewing Schedule', 'Daily Sewing Output', 'Fabric', 'Cutting', 'Sewing Output']),
  part('sew-glue.js'),
  sewBody,
  part('sew-foot.js'),
].join('\n\n'));

out('finishing/script.js', [
  part('fin-head.js'),
  LFor('fin', 'LMOD'),
  navviFor(['FINISHING', 'Finishing In', 'Finishing Status', 'F.G Shipment Plan']),
  part('fin-glue.js'),
  finBody,
  part('fin-foot.js'),
].join('\n\n'));
